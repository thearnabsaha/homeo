...
export function ChatWidget() {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      const response = await api.chat(trimmed, language);
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 h-12 w-12 rounded-full bg-foreground text-background flex items-center justify-center shadow-lg hover:bg-foreground/90 transition-colors"
      >
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-3 left-3 sm:left-auto sm:right-6 z-50 sm:w-[360px] h-[70vh] sm:h-[500px] rounded-lg border border-border bg-background shadow-xl flex flex-col animate-slide-up">
          ...
          <div className="flex items-center gap-2 p-3 border-b border-border">
            ...
            <h3 className="text-sm font-semibold flex-1">{t("chat.title")}</h3>
            <button
                      ...
                      {msg.remedies.map((r, i) => (
                        <div key={i} className="p-2 rounded bg-background/50 border border-border">
                          <div className="flex items-center gap-1.5 mb-1">
                            <span className="text-xs font-medium">{translateData(r.name, language)}</span>
